...
function Enemy(xpos, ypos, t, s)
    ...
    local initialize = function ()
        ...
        elseif self.type == "basic" then return 150, 35, 45, Animation(i_squid_basic, 280, 280, 0.35, "basic"), Animation(i_squid_basic_flipped, 280, 280, 0.35, "basic")
        elseif self.type == "fast" then return 250, 25, 25, Animation(i_squid_basic, 280, 280, 0.2, "fast"), Animation(i_squid_basic_flipped, 280, 280, 0.2, "fast")
        ...
        elseif self.type == "magala" then return 200, 100, 1000, Animation(i_magala, 560, 560, 0.45, "magala"), Animation(i_magala_flipped, 560, 560, 0.45, "magala")
    ...
    self.draw = function ()
        if self.sprite ~= nil and self.speed > 0 then
            ...
            elseif self.type == "magala" then self.sprite.draw(self.x - 115, self.y - 115)
            end
        elseif self.sprite_flipped ~= nil and self.speed < 0 then
            ...
            elseif self.type == "magala" then self.sprite_flipped.draw(self.x - 115, self.y - 115)
            end
        end

        love.graphics.setColor(0.25, 0, 0)
        love.graphics.rectangle("fill", self.x - (self.type == "magala" and 0.2 or 1) * maxHP / 2, self.y + 10 + self.radius, (self.type == "magala" and 0.2 or 1) * maxHP, 6)
        love.graphics.setColor(1, 0, 0)
        love.graphics.rectangle("fill", self.x - (self.type == "magala" and 0.2 or 1) * maxHP / 2, self.y + 10 + self.radius, (self.type == "magala" and 0.2 or 1) * maxHP * (self.health / maxHP), 6)
